...
import { Calendar, CreditCard, Image as ImageIcon, Plus, Save, ShoppingBag, Trash2 } from "lucide-react";
import { paymentMethods } from "../../utils/paymentMethods";
import QuickCreateModal from "../ui/QuickCreateModal";
import api from "../../api/axios";
import { getLoggedUser } from "../../utils/auth";

type Props = { purchase?: any | null; receiptPhoto?: File | null; locations: any[]; categories: any[]; onSave: (data: any) => void; onCancel: () => void };
...
const PurchaseFormDesktop: React.FC<Props> = ({ purchase, receiptPhoto, locations, categories, onSave, onCancel }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {receiptPhoto && <div className="flex max-w-4xl items-center gap-3 rounded-2xl border border-violet-500/30 bg-violet-600/10 px-5 py-4"><ImageIcon className="text-violet-300"/><div><p className="font-bold">Nota adicionada para leitura</p><p className="text-sm text-slate-400">{receiptPhoto.name}</p></div></div>}
